...
public partial class CreateViewModel : ViewModelBase
{
    ...
    private void ExecuteCreateJob()
    {
        try
        {
            ...
            ExecuteCancel();
            ShowSuccessMessage("Travail de sauvegarde créé avec succès");
        }
        ...
        {
            DestinationPathError = "Erreur d'accès au disque ou au dossier";
        }
        catch (Exception ex)
        {
            JobNameError = "Une erreur inattendue est survenue lors de la création du travail";
        }
    }
    ...
    public void ValidateNameOnLostFocus()
    {
        ...
        {
            JobNameError = $"Nom de sauvegarde invalide, nombre de caractères maximum : {inputMax}";
        }
        else
        {
            JobNameError = null;
